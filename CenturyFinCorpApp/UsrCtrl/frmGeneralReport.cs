...
namespace CenturyFinCorpApp.UsrCtrl
{
    public partial class frmGeneralReport : UserControl
    {
        ...
        private void CalculateIncome(bool considerSalary = false)
        {
            ...
            var companyBalance = bd.GetGenReportData(considerSalary);
            //companyBalance.BalanceDetail.AddBalanceDetails();

            finalData = companyBalance.incomeReports;

            //PROFIT & LOSS Details
            lblTotal.Text = bd.ProfitText;
            lblLoss.Text = bd.LossText;
            lblCloseCount.Text = bd.CloseCount;

            lblSalary.Visible = considerSalary;
            lblSalary.Text = $"Salary : {bd.Salary.ToMoneyFormat()}";


            // Profit Percentages.
            lblIntPerc.Text = bd.LossPercText;
